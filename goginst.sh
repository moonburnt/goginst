...
### Script that extracts and installs gogrepo-downloaded data with ease
...
#Setting up variables based on optional dependencies
optvars_setup() {
    #Checking the optional dependencies. If they arent there - switch values to false. Its kind of mess right now, will improve later
    dir_check "$extrasdir" || unpackextras=false
    file_check "$extractor" || gogext=false
    dep_check "innoextract" || inno=false
}

#Shows the general usage info and configfile entries
show_help() {
    printf "$scriptname - bash script that extracts and installs gogrepo-downloaded data with ease.

##Usage:
During the very first run, you will be asked to enter paths to your data. If you've missed/misconfigured something - see ##Configuration entry below. After configuring script once, syntax to use it is following:
$scriptname <slug-of-the-game-you-want-to-unpack>
Search expressions (like *) are supported. Only one game can be installed at once - if multiple results has been received, script will either ask you to pick one (if interactive mode is enabled) or print error message and exit.

##Configuration:
All script's configuration is done entirely via single file, located in $configpath/$configname. Common bash rules apply. If some entries are missing or non-valid - script will try to use fallback values if possible (or print error and exit, if not). If you've messed things up - you can re-create it by using -cm flag (see ##Launch flags).

##List of valid $configname entries:
#Required:
downloads=\"/path/to/dir\"      #Path to directory with gogrepo-downloaded games
gamedir=\"/path/to/dir\"        #Path to directory that will contain your installed games

#Optional:
menupath=\"/path/to/dir\"       #Path to custom directory, used to store menu entries generated by menumaker
extrasdir=\"/path/to/dir\"      #Path to directory that will contain extras, shipped with games (OSTs and so on)
extractor=\"/path/to/dir\"      #Path to gogextract.py, which is used to speed up unpacking on native linux games
...
menumaker=true                #Determines if you want to create XDG menu entries for games you've installed. Default = true

##Launch flags:
There are multiple launch flags, supported by $scriptname. General usage is the following:
$scriptname <-flag>

##List of valid launch flags:
-h/-help                      #Prints this message
-v/-version                   #Prints name and version of this script
-i/-info                      #Prints general script's into - name, version and list of used variables
-cm/-configmaker              #Forces script to re-create configfile. WARNING: it will overwrite your current setup, dont forget to back it up!
"
}

#Shows the general script's info - version and settings it runs with
show_info() {
    printf "Running $scriptname $scriptversion with following settings:
...
menu_maker() {
    ...
    #checking if we have custom menupath being set. If not - using default one instead
    if [ -z "$menupath" ]; then
        local menupath="$HOME/.local/share/applications"
    fi
    ...
    dir_check "$menupath" || return 1

    if [ -f "$menupath/$slug-goginst.desktop" ]; then
...
if [ "$#" == 0 ]; then
    printf "Input is empty. Basic usage:
$scriptname <name-of-the-game-you-want-to-install

For additional info/help see:
$scriptname -h
"
    exit 0
fi

#Checking for launch flags
if [ "$1" == "-h" ] || [ "$1" == "-H" ] || [ "$1" == "-help" ] || [ "$1" == "-HELP" ]; then
    ...
    exit 0
elif [ "$1" == "-i" ] || [ "$1" == "-I" ] || [ "$1" == "-info" ] || [ "$1" == "-INFO" ]; then
    source "$configpath/$configname" || error_and_exit "$configpath/$configname doesnt exist, please run script with -cm flag and try again"
    optvars_setup
    show_info
    exit 0
elif [ "$1" == "-cm" ] || [ "$1" == "-CM" ] || [ "$1" == "-configmaker" ] || [ "$1" == "-CONFIGMAKER" ]; then
    config_maker
    echo "Done"
    exit 0
fi
...
if [[ "$OSTYPE" == "linux-gnu" ]]; then
    ...
    error_and_exit "Your OS isnt officially supported. Abort"
fi
...
#Checking all the necessary dependencies
...
#Checking the optional dependencies and setting up variables accordingly
optvars_setup
#Once all necessary checks are done, printing the values that will be used by script. There are also optional vars checks, will sort that mess later
show_info
